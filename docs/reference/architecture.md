...
Centralizes visibility rules (`postAccess.ts` — `canViewPostPolicy`/`resolveVisiblePosts`/`canViewPost`/`requirePostAccess`) so that post/comment/like visibility (audience type, friendship/circle temporal gates, blocking, reports) is enforced in one place rather than re-implemented per route. `buildAudienceCandidateWhere()` is the shared Prisma `OR`-clause fragment for the audience half of the check, for feed/list queries that need a cheap DB-level prefilter before running the full check. New surfaces that read posts, comments, or likes should call into this module rather than re-deriving any part of visibility — full rule set in [post-visibility-rules.md](../specs/2026-08-02-post-visibility-rules.md).

### Scheduled jobs
`vercel.json` defines the cron schedule (`0 7 * * 0` — Sundays 07:00 UTC) hitting `/api/cron/publish-weekly`, which calls `publishEditionForWeek()` in `src/lib/editions.ts`. The route also accepts an admin-triggered manual run, authenticated separately from the DB `role` field via an `ADMIN_EMAILS` env allowlist — see [product-spec.md](./product-spec.md#roles) for why these two admin checks aren't the same mechanism. The cron itself is unchanged by Sunday live-join (below) — it only ever sweeps `SUBMITTED` posts, and a live-joined post is already `PUBLISHED` by the time it would run.

### Sunday live-join (bypasses the cron)
`PUT /api/posts/[id]` can move a post straight `DRAFT → PUBLISHED`, outside the weekly cron, when the request status is `"PUBLISHED"` and the server-side window/edition check passes: `getSundayLiveJoinWindow()`/`getOpenLiveJoinEdition()` in `src/lib/editions.ts` derive the 24h window and the currently-open edition from `Edition.publishedAt` (no separate "seal" field). The route re-validates this independently of whatever the client showed. See [sunday-live-join.md](../specs/2026-08-13-sunday-live-join.md).

### Notifications & delivery
`src/actions/notification.action.ts` is the single place notification events are created and fanned out to in-app rows, email (`src/lib/emails/`), and push (`src/lib/push/`), gated per-user by `NotificationPreference`. `createSubmitNotifications` and `createPublishNotifications` (the latter fired by Sunday live-join) share a `resolveAudienceRecipientIds()` helper for the audience-resolution logic.
