...
**WeeklyJamView** — composite PK `(editionId, userId)`, `openedAt`. Mirrors `EditionView`'s "opened" tracking but scoped to the Jam, since the Jam has no `Post` row of its own to hang a `PostRead` off. Stamped by `markWeeklyJamViewed()` (`src/lib/jam.ts`) when the Jam page renders. Lets the Jam participate as a pseudo-post in the reader's "Keep reading" / "Already read this week" lists (`EditionUpNext.tsx`) — read via `hasViewedWeeklyJam()`, surfaced on `getPublishedEditionById()`'s `weeklyJam.readByMe`.
