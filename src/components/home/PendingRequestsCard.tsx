...
export function PendingRequestsCard() {
  ...
  const respond = useCallback(
    ...
    [mutate, globalMutate, nudge.checkAfterAccept],
  );

  const items = data?.items ?? [];
  // Note: `nudge.prompt` must stay in this guard — accepting your last
  // pending request makes `items` empty on the very refetch that should
  // reveal the nudge below, so bailing on `items.length === 0` alone would
  // unmount the prompt before it's ever shown.
  if (isLoading) return null;
  if (items.length === 0 && !nudge.prompt) return null;
                    ...
                    {isActing ? (
                      ...
                      <>
                        <Button
                          ...
                          className="h-8 px-3"
                          onClick={() =>
                            respond(user.id, "accept", user.username ?? "")
                          }
                        >
                        ...
                        <Button
                          ...
                          className="h-8 px-3"
                          onClick={() =>
                            respond(user.id, "decline", user.username ?? "")
                          }
                        >
                          <X className="h-4 w-4" />
                          Decline
