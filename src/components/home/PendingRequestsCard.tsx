...
export function PendingRequestsCard() {
  ...
  // Note: don't fold `republishPrompt` into this guard — accepting your
  // last pending request makes `items` empty on the very refetch that
  // should reveal the prompt below, so bailing on `items.length === 0`
  // alone would unmount the prompt before it's ever shown.
  if (isLoading) return null;
  if (items.length === 0 && !republishPrompt) return null;
