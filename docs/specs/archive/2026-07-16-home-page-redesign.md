...
- **Read tracking**: new `PostRead` model (see [data-model.md](../../reference/data-model.md)), written via `POST /api/posts/[id]/read` (fired once per mount from `src/app/reader/[id]/page.tsx`, mirroring the existing `EditionView`/reveal-overlay pattern) and read via `src/lib/postReads.ts`.
