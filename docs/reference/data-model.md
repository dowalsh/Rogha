...
**User** — `email`, `username` (unique), `clerkId` (unique, links to Clerk). `role: USER | ADMIN`. `lastfmUsername` (nullable) + `jamEnabled` (bool, default `false`) back the Weekly Jam opt-in (see `WeeklyTrack` below). `onboardingIntroSeenAt` (nullable, stamped once) + `onboardingChecklistCollapsed` (bool, default `false`) back first-run onboarding's one-time full-screen intro and the dismissible-but-recoverable home checklist — see [product-spec.md](./product-spec.md#first-run-onboarding). `circleJoinBuzzSeenAt` (nullable) is the global "seen" cursor for circle-join rows in the home page Buzz list — advanced once per visit client-side (`markCircleJoinBuzzSeen()`), not on every `/api/home` read, so New/Earlier doesn't shift mid-visit; see [product-spec.md](./product-spec.md#home-page-buzz). Fans out to nearly everything else: posts, comments, likes, notifications (sent + received), friendships, circle memberships, notification preferences (1:1), push devices, reports filed, blocks given/received.
...
**WeeklyTrack** — scalar `id` PK, unique on `(editionId, userId)` (one row per opted-in user per edition). `name`, `artist`, `playCount`, `imageUrl`/`imageSource` (nullable — sourced from Spotify's catalog API, Last.fm as fallback), `spotifySearchUrl` (search-results fallback link), `spotifyTrackUrl` (nullable — exact `open.spotify.com/track/<id>` match when Spotify search found one; preferred over `spotifySearchUrl` when present), `lastfmUrl`, `capturedAt`. Populated by the Sunday cron (`captureWeeklyJamTracks()` in `src/lib/jam.ts`, called from `src/app/api/cron/publish-weekly/route.ts` right after `publishEditionForWeek()`), upserted on every cron run. Backs the "Weekly Jam" card on the Edition page and has its own `Comment` thread per row — see [product-spec.md](./product-spec.md), `docs/specs/2026-08-04-weekly-jam-mvp.md`, and `docs/specs/2026-09-06-weekly-jam-track-comments.md`.

**WeeklyJamView** — composite PK `(editionId, userId)`, `openedAt`. Mirrors `EditionView`'s "opened" tracking but scoped to the Jam, since the Jam has no `Post` row of its own to hang a `PostRead` off. Stamped by `markWeeklyJamViewed()` (`src/lib/jam.ts`) when the Jam page renders. Lets the Jam participate as a pseudo-post in the reader's "Keep reading" / "Already read this week" lists (`EditionUpNext.tsx`) and in the home page hero's opened/unread tallies (`getHeroData()` in `src/lib/home.ts`) — read via `hasViewedWeeklyJam()`, surfaced on `getPublishedEditionById()`'s `weeklyJam.readByMe`.
...
**Comment** — `status: ACTIVE | REMOVED`. Attaches to exactly one of `postId` or `weeklyTrackId` (both nullable, enforced in the API — see `docs/specs/2026-09-06-weekly-jam-track-comments.md`). One level of self-referential nesting via `parentCommentId` / `replies` (server enforces max depth 2 — top-level + one reply).
